...
アップロード後、カラムマッピング画面が表示されます。
カラム名の類似性（日本語・英語対応）に基づき自動マッピングが提案されます。ドロップダウンで手動修正も可能です。
...
| ------ | ---- | ---- |
| `journal_id` | ◎ | 伝票番号 |
| `effective_date` | ◎ | 計上日（YYYY-MM-DD） |
| `gl_account_number` | ◎ | 勘定科目コード |
| `amount` | ◎ | 金額 |
| `debit_credit_indicator` | ◎ | 借貸区分 |

**任意フィールド:**

| カラム | 説明 |
| ------ | ---- |
| `je_line_description` | 摘要 |
| `prepared_by` | 起票者 |
| `approved_by` | 承認者 |
| `dept_code` | 部門コード |
| `vendor_code` | 取引先コード |
| `fiscal_year` | 会計年度 |
| `entry_date` | 入力日 |
| `business_unit_code` | 事業部コード |

> **ヒント**: 必須項目がすべて設定されるまで「検証する」ボタンは無効です。

#### Step 3: バリデーション

「検証する」ボタンをクリックすると、データの整合性チェックが実行されます。

- **検証OK**（緑）: そのままインポート可能
- **警告あり**（黄）: 確認の上「警告を無視してインポート」可能
- **エラーあり**（赤）: エラー内容を確認し「マッピングに戻る」で修正

#### Step 4: 取込実行

「インポート実行」をクリックしてインポートを実行します。
完了後、ダッシュボードに反映されます。
...
| 目的 | 説明 |
| ---- | ---- |
| 監査人向け | 違反データの詳細、技術的指標を重視 |
| 経営層向け | サマリー重視、視覚的チャート中心 |

---

## 7.5 詳細分析

サイドバーの「詳細分析」から、3つのタブで分析できます。
...
取引先の集中度分析。取引金額・平均金額・最大金額・リスクスコア・集中度を確認できます。

### 勘定科目フロー

借方勘定と貸方勘定のペアごとの資金フローを表示します。取引件数、フロー金額、金額比率を可視化します。
