...
│  │  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘   │   │
│  │  ┌──────────────────────────────────────────────────────────────────┐   │   │
│  │  │  AutonomousAuditPage (5フェーズ進捗、ログ/仮説/インサイト/要約)  │   │   │
│  │  └──────────────────────────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                       │                                         │
│                                       ▼                                         │
...
│  │  └───────────────────────────────────────────────────┘                   │   │
│  │  ┌─────────────────────────────────────────────────────────────────┐   │   │
│  │  │  Autonomous Audit Agent (LangGraph StateGraph)                   │   │   │
│  │  │  ┌──────────┐ ┌────────────┐ ┌─────────┐ ┌────────┐ ┌───────┐  │   │   │
│  │  │  │ Observe  │→│ Hypothesize│→│ Explore │→│ Verify │→│Synthe.│  │   │   │
│  │  │  └──────────┘ └────────────┘ └─────────┘ └────────┘ └───────┘  │   │   │
│  │  │  AuditToolRegistry (13 tools) │ HITL Checkpoint │ SSE Stream   │   │   │
│  │  └─────────────────────────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                       │                                         │
│                                       ▼                                         │
...
│   │   │   ├── visualizer.py
│   │   │   └── reporter.py
│   │   ├── autonomous/           # 自律型監査エージェント
│   │   │   ├── __init__.py       # AutonomousAuditAgent
│   │   │   ├── agent.py          # LangGraph StateGraph 定義
│   │   │   └── state.py          # AuditPhase, AutonomousAuditState
│   │   ├── autonomous_tools/     # 自律型監査ツール (13種)
│   │   │   ├── __init__.py       # create_default_registry
│   │   │   ├── registry.py       # AuditToolRegistry
│   │   │   └── tools.py          # 13種分析ツール実装
│   │   ├── tools/                # エージェントツール
│   │   │   ├── dashboard_tools.py
│   │   │   ├── data_tools.py
...
```
User              Frontend              Backend API         AutonomousAuditAgent    LLM
 │                    │                      │                      │                │
 │  監査開始          │                      │                      │                │
 │───────────────────>│                      │                      │                │
 │                    │ POST /autonomous-audit/start/stream          │                │
 │                    │─────────────────────>│                      │                │
 │                    │                      │  セッション作成      │                │
 │                    │                      │─────────────────────>│                │
 │                    │  SSE: 進捗ストリーム │                      │                │
 ...
 │  SSE: phase=observe│                      │  │ account_balance_summary()    │─────│
 │<═══════════════════│                      │  │ rule_risk_summary()          │     │
 │                    │                      │  └─────────────────────────────┘     │
 │                    │                      │                      │                │
 │                    │                      │  ┌─ Phase 2: HYPOTHESIZE ──────┐     │
 │  SSE: hypotheses   │                      │  │ LLMが仮説を生成            │─────│
 │<═══════════════════│                      │  │ (3〜5件の監査仮説)          │     │
 │                    │                      │  └─────────────────────────────┘     │
 │                    │                      │                      │                │
 │                    │                      │  ┌─ HITL Checkpoint ──────────┐     │
 │  仮説承認待ち      │                      │  │ auto_approve=false の場合   │     │
 ...
 │                    │                      │                      │                │
 │                    │                      │  ┌─ Phase 3: EXPLORE ──────────┐     │
 │  SSE: tool_calls   │                      │  │ 13種ツールで仮説を探索     │     │
 │<═══════════════════│                      │  │ (stratification, duplicate,  │─────│
 │                    │                      │  │  correlation, t_account...) │     │
 │                    │                      │  └─────────────────────────────┘     │
 │                    │                      │                      │                │
 │                    │                      │  ┌─ Phase 4: VERIFY ──────────┐     │
 │  SSE: verification │                      │  │ 証拠の交差検証             │─────│
 │<═══════════════════│                      │  │ grounding_score 計算       │     │
 │                    │                      │  └─────────────────────────────┘     │
 │                    │                      │                      │                │
 │                    │                      │  ┌─ Phase 5: SYNTHESIZE ──────┐     │
 │  SSE: complete     │                      │  │ インサイト生成             │─────│
 │<═══════════════════│                      │  │ エグゼクティブサマリー作成 │     │
 │                    │                      │  │ DB永続化                   │     │
 │                    │                      │  └─────────────────────────────┘     │
 │                    │                      │                      │                │
 │  レポート表示      │ GET /{id}/report     │                      │                │
 │<───────────────────│─────────────────────>│                      │                │
```
...
- **SSE (Server-Sent Events)**: 自律型監査エージェントの5フェーズ進捗ストリーミング（`/autonomous-audit/start/stream`）
