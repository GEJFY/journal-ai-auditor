...
def execute_account_balance_analysis(
    ...
    findings.append(
        f"BS勘定残高合計: {bs_total:,.0f}円、PL勘定残高合計: {pl_total:,.0f}円"
    )

    # 大きな残高変動
    large_movements = [
        r for r in rows if abs(r["net_movement"]) > 0 and r["beginning_balance"] != 0
    ]
    large_movements.sort(key=lambda r: abs(r["net_movement"]), reverse=True)
    for r in large_movements[:5]:
        chg = (
            r["net_movement"] / abs(r["beginning_balance"]) * 100
            if r["beginning_balance"]
            else 0
        )
        findings.append(
    ...
    # マイナス残高（異常検出）
    negative_bs = [
        r for r in rows if r["account_category"] == "BS" and r["ending_balance"] < 0
    ]
    if negative_bs:
