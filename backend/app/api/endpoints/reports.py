...
def _query_agent_findings(
    ...
    query = """
        SELECT finding_id, workflow_id, agent_type, finding_title,
               finding_description, severity, category,
    ...
    """
    try:
        result = db.execute(query, [fiscal_year, limit])
        return [
            {
                "finding_id": row[0],
                "workflow_id": row[1],
                "agent_type": row[2],
                "finding_title": row[3],
                "finding_description": row[4],
                "severity": row[5],
                "category": row[6],
                "affected_amount": row[7] or 0,
                "affected_count": row[8] or 0,
...
async def get_agent_findings(
    ...
    try:
        ...
        return {"findings": [], "total_count": 0, "severity_summary": {}}
...
""".strip()
    ...
    return {
        ...
        "agent_findings": _query_agent_findings(db, request.fiscal_year, limit=10),
    }
...
async def _generate_detailed_report(
    ...
    return {
        ...
        "agent_findings": _query_agent_findings(db, request.fiscal_year),
    }
...
async def _generate_executive_report(
    ...
    return {
        ...
        "generated_for": "経営陣向け",
        "agent_findings": _query_agent_findings(db, request.fiscal_year, limit=5),
    }
